...
function build_navBlock() { 
    ...
    <div id="navBlock">
      ...
      <div class="navItem" onclick="navMan('index.php')">HOME</div>
        <div class="navItem" onclick="navMan('about.php')">ABOUT</div>
	  ...
	  <div class="navItem" onclick="navMan('shoplist.php')">SHOPPING CART</div>
	  <div class="navItem" onclick="navMan('catalog.php')">CATALOG</div>
...
/*register user activity and errors in a log files */
...
$log_msg = "LOG - IP address: " . $_SERVER['REMOTE_ADDR'] . ' - ' . PHP_EOL . date('d/m/Y H:i:s') .
": {$log_msg}" . PHP_EOL . "----------------------------" . PHP_EOL;
file_put_contents($log_file_data, $log_msg . "\n", FILE_APPEND);
}

function reg_log($uName, $msg)
{
	$log = "LOG - IP address: " . $_SERVER[REMOTE_ADDR] . ' - ' . date('d/m/Y H:i:s') . 
	PHP_EOL . "Action: " . $message . " - User: " . $uName . PHP_EOL . "-------------------------" . PHP_EOL;
	file_put_contents('./log/lpa_log'. date("j.n.Y") . '.log', $log, FILE_APPEND
	);
}
?>